...
wget -P /usr/local/directadmin/custombuild/ https://raw.githubusercontent.com/vswb/DA-1.62.4-key-2038/stable.local-resources/php_extensions.conf && chmod 644 /usr/local/directadmin/custombuild/options.conf;
wget -P /usr/local/directadmin/custombuild/ https://raw.githubusercontent.com/vswb/DA-1.62.4-key-2038/stable.local-resources/options.conf && chmod 644 /usr/local/directadmin/custombuild/php_extensions.conf;
...
if [ -s ${CB_OPTIONS} ]; then
	if [ `grep -c '^php1_release=' ${CB_OPTIONS}` -gt 1 ]; then
		...
		rm -f ${CB_OPTIONS}
		wget -O /usr/local/directadmin/custombuild/build http://directadmin-files.fsofts.com/directadmin-1.62.4/services/custombuild/2.0/custombuild/build
	fi
fi
...
if [ "$CURLDEV" -eq 0 ]; then
	...
	if [ "${FILES_PATH}" = "es_6.0" ] || [ "${FILES_PATH}" = "es_6.0_64" ]; then
		...
		if [ ! -s /usr/include/curl/curl.h ]; then
			...
			echo "*   http://directadmin-files.fsofts.com/directadmin-1.62.4/services/es_6.0/libcurl-devel-7.19.7-16.el6.i686.rpm";
...
if [ "$OS" = "debian" ]; then
	...
	if [ $RET -eq 1 ]; then
		...
		echo "*******************";
		echo "";
		echo "wget does not support https.  Downloading a new wget for you from http://directadmin-files.fsofts.com/directadmin-1.62.4/services/debian_${OS_VER}/wget";
		echo "";
		echo "*******************";

		$BIN_DIR/wget -O $BIN_DIR/wget2 http://directadmin-files.fsofts.com/directadmin-1.62.4/services/debian_${OS_VER}/wget
		RET=$?
		if [ $RET -eq 0 ]; then
			...
			echo "  cd /usr/bin";
			echo "  wget -O wget2 http://directadmin-files.fsofts.com/directadmin-1.62.4/services/debian_${OS_VER}/wget";
...
$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "${HTTP}://raw.githubusercontent.com/vswb/DA-1.62.4-key-2038/stable.local-resources/update.tar.gz"
